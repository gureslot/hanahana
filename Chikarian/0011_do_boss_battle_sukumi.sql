-- =============================================================================
-- 0011 : do_boss_battle 改修＝三すくみ有効化（ボス属性/武器・乗算方式）＋ sukumi_factor
-- 出典: boss-attr-sukumi-2026-06-14.md §1(係数1.2/0.8/1.0)・§2(B方式乗算)・§3(boss_master)
--        / balance §6-1(base_power)・§6-2/§6-3(報酬)・§9(損失)
-- 前提: 0008(do_boss_battle 初版)・0010(boss_master) 実行済み。本ファイルで do_boss_battle を上書き。
-- 実行: SQL Editor に貼って Run（create or replace・再実行可）。**0010 の後に実行すること。**
--
-- 【0008 からの差分】
--   1) 敵戦力を inline CASE → boss_master JOIN（base_power × N周目）に変更。
--   2) 三すくみ(§7-5h)を有効化：各カードの「属性係数 × 武器係数」を総合に乗算（§2・B方式）。
--      属性係数 = 敵の各属性に対する判定(有利1.2/不利0.8/中立1.0)を全乗算。武器係数も同様。
--      1面ボス(属性/武器なし)・SPカード(無属性・装備なし)は中立(1.0)。
--   3) amplify_advantage(絶対優勢)を有効化：自身が有利(係数>1.0)のとき係数に +base（仮+0.30）。
--   その他（決定的勝敗・損失§9・報酬§6・cleared_stage・SP離脱・battle_logs）は 0008 と同一。
-- =============================================================================


-- ===== 三すくみ係数ヘルパ（c=カード側 / e=敵側の単一属性 or 武器）====================
-- 有利1.2 / 不利0.8 / 中立1.0。輪: 花→芯→葉→花(hana→shin→ha) / 剣→盾→杖→剣(ken→tate→tsue)。
...
as $$
  select case
    ...
    when (c, e) in (('hana','shin'),('shin','ha'),('ha','hana'),
                    ('ken','tate'),('tate','tsue'),('tsue','ken')) then 1.2   -- c が e に有利
    else 0.8                                                                  -- e が c に有利
  end::numeric;
$$;
...
begin
  ...
  for i in 1..3 loop
    ...
    if v_is_sp then
      ...
      lvcap := 50;
      base  := base * (1 + (vc.lv - 1) * (2.0 / (lvcap - 1))) * (1 + 0.15 * vc.star);
