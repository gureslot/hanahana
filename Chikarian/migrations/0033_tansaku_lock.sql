-- ============================================================
-- Chikarian migration 0033: 占有ロック土台（cards.tansaku_deck_no）＋ start/collect 改修＋バックフィル
--   canon-06 §3-4 占有ロック / canon-07 §4「未実装」（B-2方式の第一段）
--
--   探索/放置に出ているデッキのカードは 編成・★強化・スキル転移・充填・出撃ができない。
--   判定列 = cards.tansaku_deck_no（null=未探索/自由・値=探索中デッキ番号）。前提＝1カード＝1デッキ。
--   ・出発（start_tansaku）：そのデッキの編成カードを tansaku_deck_no = deck_no でロック。
--   ・帰還（collect_tansaku＝回収）【ユーザー確認＝A】：ロック解除（null）＋探索終了（tansaku_states 削除）。
--       ＝「送り出す→連れ戻す」ループ。回収すると探索は終わり、再度回すには再出撃。
--   ・バックフィル：既に探索中のデッキの編成カードを一回限りでロック。
--
--   本ファイルは 0024 を base に start/collect を改修（depth正規化・レート等の既存ロジックは 0024 と同一）。
--   ※各資産RPC（update_deck/do_kyoka/do_skill_teni/do_skill_rensei/equip_buki/do_boss_battle/start_tansaku）への
--     CARD_IN_TANSAKU 拒否チェックは 0034（次段）。本体戦力ゲートのしきい値は仮＝再シミュ後（本ファイル非対象）。
--     放置(is_houchi)の自動再出撃は別途・未実装。
--   前提: 0018→0024（探索RPC）適用済み。本ファイルで start_tansaku/collect_tansaku を上書き。
-- ============================================================

-- 占有ロックの判定列：null=未探索(自由) / 値=探索中デッキ番号
alter table public.cards add column if not exists tansaku_deck_no smallint;
...
begin
  ...
  -- 帰還（canon-06 §3-4・ユーザー確認A）：このデッキの編成カードのロック解除（null）＋探索終了
...
-- ---- バックフィル（一回限り・冪等）：既に探索中(tansaku_states 有)のデッキの編成カードをロック ----
--   d.deck_no は smallint、tansaku_states.deck_no と decks.deck_no で突合。is_houchi 分も含めロック。
update public.cards c
   set tansaku_deck_no = t.deck_no
  from public.tansaku_states t
  join public.decks d on d.user_id = t.user_id and d.deck_no = t.deck_no
