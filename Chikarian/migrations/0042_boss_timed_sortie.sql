-- ============================================================
-- Chikarian migration 0042: ボス＝時間制出撃（start/collect/cancel）＋ 占有ロック拡張
--   canon-05 §0（ボス＝時間制出撃）／canon-06 §4・§5（出撃フロー・UI）／canon-07（schema/RPC）
--
--   【決定（2026-06-19・このセッションで確定）】
--   ボス戦を「即時」から「時間制の出撃」に変更：
--     出撃(start) → 約30分待ち（デッキは出払う＝占有ロック）→ 結果(collect)＝報告書（ボスイラスト入り）。
--   マップ/ホームでは出撃ゲージを表示。キャンセル可＝「そこから折り返し」で
--     帰還時間 = 出撃してからの経過時間（10分出ていたら帰還に10分）。帰還完了でデッキ解放・結果なし。
--   挑戦回数(3/日)は【出撃時に消費・キャンセルで返金】（完遂した戦闘だけがカウント）。
--   戦闘の入力(Lv/★/武気)は出撃中ロックで固定 → 戦闘判定は collect 時に実行（出撃時と同値）。
--   待ち時間 travel_sec は start_boss_battle 内の定数（★テスト中=30秒・本番は 1800＝30分）。
--
--   実装方針：
--    ・戦闘計算は既存 do_boss_battle を流用。引数 p_from_sortie を追加し、
--      true(=collect経由) のときは「1日3回判定・フロンティア検証・占有アサート・回数加算」をスキップ
--      （これらは出撃時 start で実施済み／占有は当該デッキ自身のため）。戦闘式そのものは一字一句不変。
--    ・占有ロックは探索の仕組みを流用：cards.boss_deck_no を追加し、共通ガード
--      _chikarian_assert_not_in_tansaku を「探索 or ボス出撃中なら拒否」に再定義。
--      → 各資産RPC（update_deck/do_kyoka/do_skill_rensei/do_skill_teni/equip_buki/start_tansaku/do_boss_battle）
--        は全てこの共通関数を呼ぶため、本体は無改変でボス出撃中カードも自動的にロックされる。
--   前提: 0033–0039 適用済み（cards.tansaku_deck_no / 共通ガード / boss_round / 現行 do_boss_battle）。
-- ============================================================

-- 1) ボス出撃の占有判定列（null=自由 / 値=出撃中デッキ番号）
alter table public.cards add column if not exists boss_deck_no smallint;

-- 2) 共通ガード再定義：探索中(tansaku_deck_no) または ボス出撃中(boss_deck_no) なら拒否
...
begin
  ...
  if exists (
    select 1 from public.cards
     where id = p_card_id
       and (tansaku_deck_no is not null or boss_deck_no is not null)
  ) then
...
-- 3) ボス出撃テーブル（1デッキ1出撃）。読みは get_boss_sorties（security definer）経由＝直接SELECT不可。
create table if not exists public.boss_sorties (
  ...
  return_until timestamptz,
  primary key (user_id, deck_no)
);
alter table public.boss_sorties enable row level security;
-- 直接アクセスは禁止（全操作は下記RPC=security definer経由）。selectポリシーは置かない。
...
-- ============================================================
-- start_boss_battle: 出撃（検証＋回数消費＋デッキ占有ロック＋出撃行作成）。戦闘はまだ起こさない。
-- ============================================================
...
declare
  ...
  cid        uuid;
  vc         public.cards;
  v_travel_sec int := 30;   -- ★テスト中=30秒。本番は 1800（30分）に変更すること。
begin
...
-- ============================================================
-- collect_boss_result: 帰還後に結果を受け取る。
--   ・通常帰還完了 → do_boss_battle(..., true) で戦闘解決＋報酬適用＋報告書 → ロック解除・行削除。
--   ・キャンセル帰還完了 → ロック解除・行削除・結果なし（{canceled:true}）。
--   ・まだ出撃中/帰還中 → エラー。
-- ============================================================
...
begin
  ...
  if v_s.is_returning then
    if now() >= v_s.return_until then
      ...
      return jsonb_build_object('canceled', true, 'deck_no', p_deck_no);
    else
  ...
  -- 通常：まだ到着していない
  if now() < v_s.started_at + (v_s.travel_sec || ' seconds')::interval then
    raise exception 'STILL_OUT';
  end if;

  -- 帰還完了 → 戦闘解決（出撃モード：回数/フロンティア/占有はスキップ）
  ...
  return v_res || jsonb_build_object('canceled', false);
end;
$$;
...
-- ============================================================
-- cancel_boss_sortie: 出撃中のみキャンセル。帰還時間 = 出撃してからの経過時間。回数を返金。
--   （帰還完了は別途 collect_boss_result で解放・結果なし）
-- ============================================================
create or replace function public.cancel_boss_sortie(p_deck_no integer)
returns jsonb
language plpgsql
security definer
set search_path = public, pg_temp
...
begin
  ...
  select * into v_s from public.boss_sorties
    where user_id = uid and deck_no = p_deck_no for update;
  if not found then raise exception 'NOT_ON_SORTIE'; end if;
  if v_s.is_returning then raise exception 'ALREADY_RETURNING'; end if;
  if now() >= v_s.started_at + (v_s.travel_sec || ' seconds')::interval then
    raise exception 'CANNOT_CANCEL_COMPLETED';   -- 到着済み＝collectで結果を受け取る
  end if;

  v_elapsed := now() - v_s.started_at;
  update public.boss_sorties
     set is_returning = true, return_until = now() + v_elapsed
   where user_id = uid and deck_no = p_deck_no;

  -- 挑戦回数を返金（戦闘が起きないため。当日カウントのみ調整）
  select * into prof from public.profiles where id = uid for update;
  if prof.boss_date = today and prof.boss_count_today > 0 then
    update public.profiles set boss_count_today = boss_count_today - 1 where id = uid;
  end if;

  return jsonb_build_object(
    'deck_no',      p_deck_no,
    'returning',    true,
    'return_until', now() + v_elapsed,
    'return_sec',   extract(epoch from v_elapsed)::int
  );
end;
$$;
revoke all on function public.cancel_boss_sortie(integer) from public, anon;
grant execute on function public.cancel_boss_sortie(integer) to authenticated;

-- ============================================================
-- get_boss_sorties: 自分の出撃中一覧（ゲージ描画用）。サーバ時刻 now() と残り秒・フェーズを返す。
--   phase: out=出撃中 / arrived=到着(回収可) / returning=帰還中(キャンセル) / return_done=帰還完了(回収で解放)
-- ============================================================
create or replace function public.get_boss_sorties()
returns jsonb
language plpgsql
security definer
set search_path = public, pg_temp
...
begin
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
    ...
    'travel_sec',   s.travel_sec,
    'arrive_at',    s.started_at + (s.travel_sec || ' seconds')::interval,
    'returning',    s.is_returning,
    'return_until', s.return_until,
    'phase', case
               when s.is_returning and v_now >= s.return_until then 'return_done'
               when s.is_returning then 'returning'
               when v_now >= s.started_at + (s.travel_sec || ' seconds')::interval then 'arrived'
               else 'out'
             end,
    'remaining_sec', case
               when s.is_returning then greatest(0, ceil(extract(epoch from (s.return_until - v_now)))::int)
               else greatest(0, ceil(extract(epoch from (s.started_at + (s.travel_sec || ' seconds')::interval - v_now)))::int)
             end
  ) order by s.deck_no), '[]'::jsonb) into v
  from public.boss_sorties s where s.user_id = uid;
  return jsonb_build_object('now', v_now, 'sorties', v);
end;
$$;
revoke all on function public.get_boss_sorties() from public, anon;
grant execute on function public.get_boss_sorties() to authenticated;
