-- =============================================================================
-- 0049 : 引き分け（自軍戦力 = 敵戦力）を「勝ち」→「敗北扱い」に修正（表示のみ Draw）
--   依頼・canon：引き分けは未クリア＝敗北と同じ。表示だけ Draw。
--   実装 0046 まで win := v_own >= v_enemy（＝タイで勝ち＝クリア）→ > に修正（タイは敗北）。
--   併せて draw フラグ（v_own = v_enemy）を返り値と battle_logs.rewards に追加（クライアントが Draw 表示に使う）。
--   ※武気損失等の敗北処理はタイにも適用（v_R=1.0 → loss_rate=0.5）。
--   ※do_boss_battle のみ再定義。start_boss_battle は 0046 のまま不変。前提：0046 適用済み。再実行可。
--   ※クライアント：報告書/結果で rewards.draw（または draw）が真なら「Draw」を表示する小改修が別途必要。
-- =============================================================================
