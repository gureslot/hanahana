-- =============================================================================
-- 0032 : do_boss_battle ギア枠攻撃力(qatk) 7/15/33/73 → 10/15/22/34（canon-04 §3 / canon-07 §3）
--   ★ギア(2/2)：装備項 = loaded_buki × qatk の qatk を新値へ。枠コスト側は 0031(equip_buki)＝同時適用。
--   本ファイルは 0026（★式 0.10）の do_boss_battle を base に qatk のみ変更＝★式0.10を引き継ぐ
--   （0032 が do_boss_battle の最新版になる）。↓以下は 0026 由来の★式説明（本ファイルも同内容を含む）：
-- 0026 : do_boss_battle ★式（係数 0.15→0.10 ＋ 適用先 本体のみ→(装備項+本体)）を内包
--   canon-04 §1 / canon-03 §4 step4 / canon-07 §3「要追従」（★式）
--
--   旧（廃止）: 本体 = レア基礎 × Lv式 × (1 + 0.15×★)   ← ★は本体のみに乗る・装備項に乗らない
--   新（canon）: カード総合 = (装備項 + 本体) × (1 + 0.10×★)   ← ★はカード総合（装備項込み）に乗る・本体非依存
--   ※「係数 0.15→0.10」と「乗せる対象を 本体のみ→(装備項+本体)」の2点セット。
--     係数だけ直して適用先が本体のままだと★の効きが設計の数分の一になる（要注意）。
--   ※ canon-03 §4 の処理順 step4「カード総合=装備項+本体」がまさに★を乗せる点。
--     hontai_pct/soubi_pct で skill 修正後の (装備項+本体) に ×(1+0.10★) を掛ける＝
--     カード総合=(装備項×(1+soubi)+本体×(1+hontai))×(1+0.10★)。
--
--   【0011 からの差分（本ファイルの変更点・これだけ）】
--     1) カードループの base から `×(1+0.15★)` を除去（body は Lv式のみの純・本体に）。SP/非SP両方。
--     2) temp 表 _bc に star 列を追加し、各カードの ★ を保持。
--     3) step4（旧253行 `sougou = equip + body`）を `sougou = (equip+body)×(1+0.10★)` に変更。
--   【本ファイルで“触らない”もの（別の要追従＝別マイグレーション）】
--     竜気覚醒 ×1.90（0006 skill_master の base_value／本ファイルは rec.val を読むだけ）／
--     三すくみ方向 剣>盾（sukumi_factor）／ギア枠攻撃力 7/15/33/73（qatk）／ボス報酬・敵戦力。
--     → これらは 0011 のまま据え置き（スコープ＝★式のみ）。
--
-- 前提: 0008→0011（do_boss_battle）・0010（boss_master）実行済み。本ファイルで do_boss_battle を上書き。
-- 実行: SQL Editor に貼って Run（create or replace・再実行可）。
-- =============================================================================


-- ※ sukumi_factor（三すくみ係数ヘルパ）は本ファイルでは再定義しない（0011 で定義済み・DBに存在）。
--   三すくみ武器の方向修正（剣>盾→盾>剣）は別マイグレーションで sukumi_factor を更新する。
--   本ファイルのスコープは★式（do_boss_battle の再定義）のみ。do_boss_battle は実行時に
--   public.sukumi_factor を呼ぶだけなので、ここで再定義する必要はない。
...
-- =============================================================================
-- 備考:
--  * 三すくみは boss_master.attrs/weapons の各要素に sukumi_factor を全乗算（§2-B方式）。
--    複数属性ボス（8面・2_boss）は有利/不利が相殺＝1枚で尖った有利を取りにくい（強敵化）。
--  * amplify_advantage(+0.30) は新係数(有利1.2)前提の妥当性を実プレイで調整（数値は仮・spec §5）。
--  * 報酬・敵戦力は balance §6。N周目は base_power×N（属性/武器は周回でも同じ）。
-- =============================================================================
