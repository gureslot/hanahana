-- =============================================================================
-- 0046 : 離脱中SPカードを「弾く」のではなく「外して出撃」できるようにする
--   従来 start_boss_battle / do_boss_battle は SP離脱中カードを含むと raise（出撃不可）。
--   本修正で、離脱中SPはその出撃から自動除外する：
--     ・start_boss_battle … 検証で raise しない／デッキ占有ロックの対象外（連れて行かない）。
--     ・do_boss_battle    … チーム構築ループで continue＝戦力にもスキルにも含めない。
--   残り2枚（または1枚）で戦闘解決。全員が離脱/空なら戦力0＝敗北（coalesce(sum,0)で安全）。
--   ※ デッキ自体は不変（SPは離脱明けに自動で再び戦列へ）。探索中カードは従来どおり _chikarian_assert_not_in_tansaku で拒否。
--
--   ベース：do_boss_battle=0042（3引数版）/ start_boss_battle=0044（役割別travel・3回制限OFFのテスト値を継承）。
--   実行：SQL Editor に貼って Run（create or replace・再実行可）。前提：0042 適用済み。0044/0045 とは独立に適用可。
-- =============================================================================
