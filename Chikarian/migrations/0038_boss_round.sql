-- ============================================================
-- Chikarian migration 0038: boss_round（周回フロンティア）＝列追加＋do_boss_battle 検証/前進
--   canon-06 §4 / canon-07 §4「未実装」（ボス周回エンドレス）
--
--   profiles.boss_round(1〜・既定1)・boss_round_stage(0〜8・既定0) を追加し、周回到達点を記録。
--   cleared_stage(0〜8・据置) は恒常解放（デッキ数/ギア/探索）を引き続き駆動＝別物。
--   do_boss_battle に：
--     (1) 出撃可否の検証＝到達点以下のみ：v_round < boss_round（過去周＝再戦可）
--         or (v_round = boss_round and v_stage <= boss_round_stage+1)（現周の先頭未クリア面まで）。
--         それ以外は BOSS_LOCKED。
--     (2) フロンティア前進＝先頭面の面ボス勝利で boss_round_stage+1、8到達で boss_round++・stage=0。
--   ★報酬には一切触らない（毎勝利フル報酬のまま）。周回報酬の価値づけ（farm報酬等）は balance/再シミュ側。
--   敵戦力 ×周回 は既存（boss_key の _r{N} を検証後に使用）。
--
--   do_boss_battle を 0037 base に再定義（★式0.10・qatk新値・占有ロック出撃チェック・恩寵石+1 を保持）。
--   前提: 0037 適用済み。本ファイルで do_boss_battle を上書き＋profiles 列追加。
-- ============================================================

-- 周回フロンティア列（既定＝round1/stage0）。cleared_stage とは別物（恒常解放用は据置）。
alter table public.profiles add column if not exists boss_round       smallint not null default 1;
alter table public.profiles add column if not exists boss_round_stage smallint not null default 0;

-- バックフィル：既存プレイヤーのフロンティアを 1周目進捗(cleared_stage)に合わせる。
--   既定 stage0 のままだとクリア済み面に出られずロックアウトするため必須。
--   cleared_stage<8 → round1・stage=cleared_stage（先頭は cleared_stage+1 面）／
--   cleared_stage=8（1周目完了）→ round2・stage0（次は2周目の先頭面）。
update public.profiles
   set boss_round       = case when cleared_stage >= 8 then 2 else 1 end,
       boss_round_stage = case when cleared_stage >= 8 then 0 else cleared_stage end;

-- ===== do_boss_battle（0037）＋ boss_round 検証/前進 =====
