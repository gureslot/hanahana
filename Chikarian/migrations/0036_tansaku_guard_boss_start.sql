-- ============================================================
-- Chikarian migration 0036: 占有ロック拒否チェック（CARD_IN_TANSAKU）を資産RPCに追加（3/3・最終）
--   canon-06 §3-4 占有ロック / canon-07 §4「未実装」（B-2方式・完了）
--
--   0034 のヘルパ _chikarian_assert_not_in_tansaku(card_id) を使う（0034 適用済み前提）。
--   ・do_boss_battle(0032 base)：出撃時にデッキ3枚が探索中なら拒否（探索カードでボス戦不可）。
--       slot_ids 確定直後にチェック。★式0.10・ギアqatk(10/15/22/34) 等は 0032 のまま保持。
--   ・start_tansaku(0033 base)：このデッキの編成カードが既に他デッキで探索中なら出発を拒否
--       （1カード＝1デッキの担保）。既存探索の回収（このデッキ分の解錠）後・出発ロック前にチェック。
--
--   これで CARD_IN_TANSAKU が全7RPC（update_deck/do_kyoka/do_skill_teni/do_skill_rensei/
--   equip_buki/do_boss_battle/start_tansaku）に揃い、占有ロック（canon-07 §4 B-2）が完成する。
--   ※注：編成時の「1カード＝複数デッキ同時所属」自体は禁じない（canon前提）。出発時の本自己チェックで
--     二重出撃を防ぐ。厳密な編成時強制が要れば別途。
--   前提: 0032・0033・0034 適用済み。本ファイルで do_boss_battle / start_tansaku を上書き。
-- ============================================================

-- ===== do_boss_battle（0032）＋ 出撃時 CARD_IN_TANSAKU =====
...
-- ===== start_tansaku（0033）＋ 出発時 自己 CARD_IN_TANSAKU =====
...
begin
  ...
  -- 直前の回収でこのデッキ分の解錠は済むので、ここで残るロックは他デッキ由来＝二重出撃の防止。
  if exists (
