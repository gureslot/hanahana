-- ============================================================
-- Chikarian migration 0039: SP本体 3,200（do_boss_battle）— SP改定(2026-06-18)の本体反映
--   canon-02 §5/§6・SP改定(2026-06-18・ユーザー承認済み)／canon-07 §3「要追従」
--
--   SP3種の本体基礎を 3,200 同値へ（旧 降順 dragon2,260/girl2,160/houou2,060＝廃止）。
--   満Lv(50)で本体MAX≈9,600（満装備SSR相当）。do_boss_battle 内の SP base CASE を 3,200×3 に。
--   竜気4.0/哀慟0.25 は 0027（skill_master）で反映済み＝本ファイルは本体のみ。
--   ★式(総合×(1+0.10★))・qatk・装備項算出は不変（SPは装備項0なので 総合=本体×(1+0.10★)）。
--
--   do_boss_battle を 0038 base に再定義（boss_round 検証/前進・★式0.10・qatk・出撃チェック・恩寵石+1 を保持）。
--   ※boss_round の列追加/バックフィルは 0038 の所掌＝本ファイルでは再宣言しない（関数定義のみ）。
--   前提: 0038 適用済み。本ファイルで do_boss_battle を上書き。
--   別途: client SP_BASE（index.html）と _chikarian_card_sopower（0020）も SP本体3,200＝別反映。
-- ============================================================

-- ===== do_boss_battle（0038）＋ SP本体3,200 =====
