-- =============================================================================
-- 0084 : claim_saishu(n) ＝ 採取RPC（日次上限を周回スケーリング：1000 × boss_round）
-- 出典: 0003 をベースに DAILY_MAX を 1000×周回数 へ。1周目=1000 / 2周目=2000 / 3周目=3000 …
--       PER_BLINK=10・0時リセット(JST)・行ロックの二重付与防止は 0003 のまま不変。
...
declare
  ...
  DAILY_MAX integer;                     -- balance §8（周回スケーリング：下で 1000×周回数 を設定）
...
begin
  ...
  -- 採取の日次上限＝1000 × 周回数（boss_round）。1周目=1000 / 2周目=2000 / 3周目=3000 …
  DAILY_MAX := 1000 * greatest(coalesce(prof.boss_round, 1), 1);
...
insert into public.schema_migrations (version) values ('0084') on conflict (version) do nothing;
