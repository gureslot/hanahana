-- ============================================================
-- Chikarian migration 0037: 恩寵石供給（面ボス初回撃破で hoshou_stone += 1）
--   canon-06 §6 / canon-04 §2（恩寵石供給＝確定・新規）
--
--   面ボス（v_role='boss'）を1周目（v_round=1）で初めて撃破（v_stage = cleared_stage+1）した瞬間に
--   profiles.hoshou_stone += 1。1周目8面で計8個（canon-04 §2 のD38デッキに要る5個＋余裕3）。
--   ＝cleared_stage 前進と同じ条件・同じ update に乗せる（初回限定＝周回では増えない）。
--   周回トリクル（週1）は達成/週次ミッション側＝別途（mission_master/claim_mission・本ファイル非対象）。
--
--   do_boss_battle を 0036 base に再定義（★式0.10・qatk新値・占有ロック出撃チェックは保持）。
--   前提: 0036 適用済み。本ファイルで do_boss_battle を上書き。boss_round（周回フロンティア）は別途 0038。
-- ============================================================

-- ===== do_boss_battle（0036）＋ 恩寵石供給 =====
