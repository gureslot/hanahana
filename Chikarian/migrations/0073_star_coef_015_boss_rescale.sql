-- ============================================================
-- Chikarian migration 0073: ★係数 0.10 -> 0.15 ＋ ボス面6-8 上げ直し（task A）
--   土台 = 0072_skill_per_lv_taper（per_lv逓減入り do_boss_battle）を verbatim 複製し、★係数のみ変更。
--   ※0071 は auto_houchi が使用済みのため、本★0.15は 0073 に採番。
--   カード総合 = (装備項 + 本体) × (1 + 0.15×★)（canon-04 §1）。全カード共通＝1周目も強化。
--   その強化分だけ 1周目ボスの面6-8 を同率上げ直し（面1-5は代表デッキ★0で据え置き）。
--   面6=★1×1.0455 / 面7・面8=★2×1.0833。M列不変＝勝率保存（忠実リゾルバ実測 差<=0.1pt）。
--   連動必須: index.html の computeStats(1415) と 武気プレビュー(4240) を 0.15 に。
--   create or replace ＋ boss_master UPDATE・再実行可。前提: 0066/0072 適用済み。
--   Supabase で手動適用後 schema_migrations に 0073 を登録。
-- ============================================================
