-- ============================================================
-- Chikarian migration 0075: 報告書用に敵戦力・発動スキルLvを battle_logs へ保存
--   目的: 報告書（VICTORY）で「VS 右＝敵戦闘力」「発動スキルのLv表記」を出すための保存項目追加。
--   背景: 0074 までの do_boss_battle は enemy_power を返り値にしか含めず battle_logs 未保存、
--          fired_skills も {owner_idx, skill_key, effect} のみで skill_lv 未保存だった。
--   変更点（0074 を verbatim 複製し、以下の最小追加のみ。戦闘ロジック・数値・勝敗は一切不変）:
--     1. 一時表 _cand に skill_lv 列を追加。
--     2. _cand への INSERT に cs.skill_lv を投入。
--     3. fired_skills スナップショット(v_fired) に 'skill_lv' を追加。
--     4. 保存用 rewards(v_rewards) に 'enemy_power'（=round(v_enemy)）を追加。
--   反映範囲: 本マイグレーション適用後の新規戦闘から保存。既存ログは enemy_power/skill_lv 無し
--             ＝報告書側はフォールバック（敵戦力「—」・Lv非表示）。
--   create or replace ＋ 一時表は on commit drop ＝ 再実行可（idempotent）。前提: 0074 適用済み。
--   Supabase で手動適用後 schema_migrations に 0075 を登録。
-- ============================================================
