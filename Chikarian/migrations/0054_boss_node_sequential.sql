-- ============================================================
-- Chikarian migration 0054: ボス面内ノードの順次解放（中A→中B→面ボス）
--   従来は「解放面の全ノードが出撃可」＝面ボスだけ直接叩いて中ボスを飛ばせ、
--   最速で各面1勝（面ボスのみ）→ 8面を数日で踏破できてしまっていた。
--   本マイグレーションで、フロンティア面（現周の先頭未クリア面）では
--     中ボスA を撃破 → 中ボスB 解放 → 中ボスB 撃破 → 面ボス 解放
--   の順次クリアを必須にする（過去面・過去周は再戦自由＝全役割可）。
--   進捗は profiles.boss_round_role（0=未/1=中A済/2=中B済）で保持し、
--   面ボス撃破でフロンティア前進と同時に 0 リセット。
--   サーバ強制：start_boss_battle（出撃ガード）＋ do_boss_battle（進捗記録＋防御ガード）。
-- ============================================================

-- 1) 面内役割の進捗列（フロンティア面で何役クリア済みか）。既定0＝中Aから。
alter table public.profiles add column if not exists boss_round_role smallint not null default 0;
-- 既存プレイヤーはフロンティア面を中Aから順次（記録が無いため0で開始＝中A再戦が必要な場合あり）。
update public.profiles set boss_round_role = 0 where boss_round_role is null;

-- ============================================================
-- 2) do_boss_battle：面内順次ゲート（防御）＋勝利時の役割進捗・前進リセット
--    （0049 をベースに boss_round_role 関連のみ追加）
-- ============================================================
...
-- ============================================================
-- 3) start_boss_battle：出撃時の面内順次ゲート（0046 をベースに追加）
-- ============================================================
