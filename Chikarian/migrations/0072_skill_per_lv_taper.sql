-- ============================================================
-- Chikarian migration 0072: スキル per_lv 逓減（Lv6以降の伸び×0.4・青天井）
--   ※当初0071想定だったが0071=auto_houchiと衝突のため0072へ採番。
--   土台 = 0066 を verbatim 複製し、_cand のスキル値計算（per_lv）のみ変更。
--   eff(Lv) = Lv<=5:(Lv-1) / Lv>=6: 4 + 0.4*(Lv-5)。Lv5で連続・1周目(Lv<=5)は無影響。
--   高Lv暴走の抑制（旧 Lv12 中立 M_eff 中央 ~6.3 -> ~4.6）。canon-03 §2。
--   create or replace・再実行可。前提: 0066 適用済み。
--   Supabase で手動適用後 schema_migrations に 0072 を登録。
-- ============================================================
