-- ============================================================
-- Chikarian migration 0017: equip_buki (装備充填)
...
--   質の枠コスト/枠攻撃力: crude 1/7 ・ refined 2/15 ・ enchanted 4/33 ・ holy 8/73。
...
begin
  ...
  v_new_cost := case p_quality when 'crude' then 1 when 'refined' then 2 when 'enchanted' then 4 when 'holy' then 8 end;
  v_new_atk  := case p_quality when 'crude' then 7 when 'refined' then 15 when 'enchanted' then 33 when 'holy' then 73 end;
  ...
  v_old_cost := case v_card.quality
                  when 'crude' then 1 when 'refined' then 2
