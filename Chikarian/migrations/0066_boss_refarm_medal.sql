-- ============================================================
-- Chikarian migration 0066: 再戦/周回ボスのメダル抑制
--   土台=0054 を verbatim 複製し、メダル決定のみ初回クリア
--   （1周目フロンティア前進撃破）限定に変更。再戦・2周目以降は medal=0。
--   EXP本・恩寵石（初回のみ）・cleared_stage/boss_round 前進は不変。
--   create or replace・再実行可。前提: 0054 適用済み。
--   Supabase で手動適用後 schema_migrations に登録。
-- ============================================================
